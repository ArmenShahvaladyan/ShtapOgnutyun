...
class SOIndicator: UIView {
    static let sharedInstance = SOIndicator()
    ...
    @IBInspectable var dotsRadius :CGFloat = 7 {
        didSet {
    ...
    @IBInspectable var dotsSpacing :CGFloat = 7 {
        didSet {
            self.setNeedsLayout()
        }
    }
    ...
    convenience init(view: UIView) {
        self.init()
        containerView = view
        self.frame = CGRect.init(x: 0, y: 0, width: view.frame.width, height: view.frame.height)
        self.backgroundColor = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0.7)
        self.tintColor = UIColor.soOrangeColor
    }
    ...
    static func showInView(_ view: UIView) {
        sharedInstance.frame = CGRect.init(x: 0, y: 0, width: view.frame.width, height: view.frame.height)
        sharedInstance.backgroundColor = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0.7)
        sharedInstance.startAnimating()
        sharedInstance.tintColor = UIColor.soOrangeColor
